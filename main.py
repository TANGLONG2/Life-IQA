...
def parse_option():
    ...
    parser.add_argument(
        "--cfg",
        default="configs/vit/deiqt_livec.yaml",
        type=str,
        required=True,
        metavar="FILE",
    ...
    parser.add_argument("--data-path", default="Datasets/ChallengeDB_release/ChallengeDB_release",type=str, help="path to dataset")
    parser.add_argument(
        "--zip",
        action="store_true",
    ...
    parser.add_argument("--tag", default='0929225_test',help="tag of experiment")
    ...
    parser.add_argument("--num_experts", type=int, help="num_experts")
...
def main(config):
    ...
    if dist.get_rank() == 0: 
        ...
        swanlab_name = group_name + "_" + str(config.EXP_INDEX)
        ...
        swanlab_runer.log({"Validating SRCC": 0.0, "Validating PLCC": 0.0, "Epoch": 0})  
    else:
        swanlab_runer = None
    ...
    if dist.get_rank() == 0:
        swanlab_runer.log("Run Finished")
        swanlab_runer.log(
            {
               "Max PLCC": max_plcc,
                "Max SRCC": max_srcc,
...
def train_one_epoch(
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        with torch.amp.autocast('cuda', enabled=config.AMP_ENABLE): 
            outputs,aux_loss,z_loss = model(samples)
        ...
        alpha = 1e-2
        beta = 1e-3
        loss = task_loss + alpha * aux_loss + beta * z_loss
    ...
    try:
        ...
        N = pred_scores.numel() // config.DATA.PATCH_NUM * config.DATA.PATCH_NUM
        pred_scores = pred_scores[:N]
        gt_scores = gt_scores[:N]
        # ----------------------------------------------
        pred_scores = (
            (pred_scores.reshape(-1, config.DATA.PATCH_NUM)).mean(dim=-1)
        ).squeeze()
        gt_scores = (
            (gt_scores.reshape(-1, config.DATA.PATCH_NUM)).mean(dim=-1)
        ).squeeze()
...
@torch.no_grad()
def validate(config, data_loader, model, epoch, val_len, swanlab_runner=None):
    criterion = torch.nn.SmoothL1Loss()
    model.eval()
    batch_time = AverageMeter()
    loss_meter = AverageMeter()
    ...
    for idx, (images, target) in enumerate(data_loader):
        ...
        temp_pred_scores.append(output.view(-1).contiguous())
        temp_gt_scores.append(target.view(-1).contiguous())
    ...
    if torch.distributed.is_initialized():
        ...
        gather_preds = (
            (gather_preds.view(-1, config.DATA.PATCH_NUM).contiguous()).mean(dim=-1)
        ).squeeze()
        ...
        gather_grotruth = (
            (gather_grotruth.view(-1, config.DATA.PATCH_NUM).contiguous()).mean(dim=-1)
        ).squeeze()
